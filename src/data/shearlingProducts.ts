...
export type ShearlingProduct = {
  ...
  coating: string | null
  origin: string | null
  variants: ShearlingVariant[]
}

const SUBTYPES = new Set(['Керли', 'Меринос', 'Тоскана', 'Тиградо', 'Меринилло', 'Кашемир', 'Izlanda'])
...
const parseSubtypes = (categories: string[]) =>
  [...new Set(
    categories
...
const catalogRecords = records.filter(record =>
  /\/dublyonka(?:\/[^/?]+)?\/tproduct\//.test(get(record, 'Url')),
)
...
export const shearlingProducts: ShearlingProduct[] = parents.map(record => {
  ...
  return {
    ...
    categories,
    subtype: parseSubtypes(categories),
...
export const shearlingImportStatistics = {
  rows: catalogRecords.length,
  products: shearlingProducts.length,
