...
export type ShoeLeatherVariant = {
  id: string
  title: string
  unit: string | null
  shade: string | null
...
export type ShoeLeatherProduct = {
  id: string
  slug: string
  title: string
  legacyUrl: string
  ...
  coating: string | null
  origin: string | null
  variants: ShoeLeatherVariant[]
}

const SUBTYPES = new Set(['Обувная'])
...
const catalogRecords = records.filter(record =>
  /\/obuvnayakozha(?:\/[^/?]+)?\/tproduct\//.test(get(record, 'Url')),
)
...
const parsedShoeLeatherProducts: ShoeLeatherProduct[] = parents.map(record => {
...
export const shoeLeatherProducts: ShoeLeatherProduct[] = parsedShoeLeatherProducts

export const shoeLeatherImportStatistics = {
  rows: catalogRecords.length,
  products: shoeLeatherProducts.length,
