...
const publicAnalyticsEvents = new Set([
  'page_view',
  'product_view',
  'variant_select',
  'add_to_cart',
...
const publicAnalyticsMetadataKeys = new Set(['category', 'quantity', 'itemCount', 'productId'])
...
function safeAnalyticsMetadata(value) {
  ...
  return Object.fromEntries(
    Object.entries(value)
      .slice(0, 12)
      .flatMap(([key, item]) => {
        if (!publicAnalyticsMetadataKeys.has(key)) return []
...
app.post('/api/analytics/events', wrap(async (req, res) => {
  ...
  if (!sessionId || !publicAnalyticsEvents.has(eventName)) return res.status(400).json({ error: 'invalid_event' })
  ...
  if (eventName !== 'heartbeat') {
    await query(
      ...
      [
        ...
        eventName,
        pathValue || null,
        String(req.body?.entityType ?? '').slice(0,80) || null,
        String(req.body?.entityId ?? '').slice(0,160) || null,
        safeAnalyticsMetadata(req.body?.metadata),
      ],
    )
  }
