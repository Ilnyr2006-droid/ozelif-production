...
function ProductImage({ product, priority = false, size = 'card' }: { product: PublicCatalogProduct; priority?: boolean; size?: 'card' | 'detail' }) {
  ...
  return <img src={responsive.src} srcSet={responsive.srcSet} sizes={responsive.sizes} alt={image.alt ?? `${product.title} — натуральная одежная кожа`} loading={priority ? undefined : 'lazy'} fetchPriority={priority ? 'high' : undefined} decoding="async" onError={() => setFailed(true)}/>
}
...
export function ClothingLeatherCatalogPage() {
  ...
  useEffect(() => {
    document.title = 'Одежная кожа — каталог натуральной кожи OZELIF'
    setMeta('meta[name="description"]', 'name', 'description', 'Одежная кожа OZELIF: гладкая, винтажная, фактурная, перфорированная и КРС. Реальные товары, оттенки и варианты из каталога.')
