...
export function createWholesaleLeadsRouter() {
  const router = Router()
  ...
  router.post(
    ...
    asyncRoute(async (request, response) => {
      ...
      const phone = text(request.body?.phone, 80)
      ...
      const result = await query(
        ...
        [
          ...
          phone,
          normalizedPhone,
          text(request.body?.company, 240),
          text(request.body?.city, 160),
          text(request.body?.category, 160),
          text(request.body?.volume, 240),
      ...
      return response.status(201).json({
        ok: true,
        lead: {
          number: Number(result.rows[0].public_number),
