...
export function deterministicCatalogReply(
  ...
  if (!Array.isArray(products) || !products.length) {
    return (
      'По вашему запросу я пока не нашёл точного совпадения '
      + 'в опубликованном каталоге. Можете добавить любые важные '
      + 'для вас требования, и я попробую подобрать точнее.'
    )
  }
  ...
  return [
    ...
    ...lines,
    '',
    'Если нужно, могу сравнить эти варианты по данным каталога.',
  ].join('\n')
}
